...
async function main(): Promise<void> {
  ...
  const preview = Bun.spawn(
    [process.execPath, "run", "preview", "--", "--port", String(PREVIEW_PORT)],
    {
      cwd: ROOT,
      stdout: "pipe",
      stderr: "pipe",
  ...
  try {
    ...
    const urls = routes.map((route) => routeToPreviewUrl(route, PREVIEW_PORT));
    console.log(`▶ pa11y em ${urls.length} rota(s): ${routes.join(", ")}`);
    ...
    const pa11y = Bun.spawn([pa11yBin, "--config", pa11yConfig, ...urls], {
      ...
      env: {
        ...process.env,
        ...(chromePath
          ? { CHROME_PATH: chromePath, PUPPETEER_EXECUTABLE_PATH: chromePath }
          : {}),
      },
    });
